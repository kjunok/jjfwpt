<template>
  <div>
            ...
            <Form style="margin: 0px 0px -10px;">
              <FormItem>
                <Input class="searchInput" v-model="keyword" style=""
                  ...
                  <Button slot="append" icon="ios-search" style="width: 70px;"  @click="api_loadUserInfoInfoList(1)"></Button>
                </Input>
              </FormItem>
            </Form>
          </Col>
    ...
    <UserForm v-on:UserFormListen="UserFormListen" v-bind:handle="handle" :show="userForm"></UserForm>
  </div>
</template>
<script>
  ...
  export default {
    ...
    data(){
      return {
        ...
        gridColumns: TUserInfo.gridColumns(this),
        gridPageData: {
          total: 100,
          start: 0,
          size: 10
        },
        searchColumns: TUserInfo.searchColumns(),
        keyword:"",
        searchField: TUserInfo.searchColumns()[0].value,
    ...
    created() {
      ...
      * 窗体高度改变处理
      */
      CommonUtil.RegHeightChangeFun("userInfo",function (height) {
        _this.clientHeight=height-150;
      });
      /*============================================================================*/
      _this.api_loadUserInfoInfoList();
    },
    destroyed(){
      CommonUtil.UnRegHeightChangeFun("userInfo");
    },
    methods:{
      api_loadUserInfoInfoList:function (start) {
          let _this = this;
          let param ={};
          if(_this.keyword){
          ...
          ReqUtil.doPost(API.userService.searchUserList.path)
            .setData(param)
