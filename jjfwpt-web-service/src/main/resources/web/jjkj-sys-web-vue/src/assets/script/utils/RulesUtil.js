...
function isMoney(str) {
  ...
  let result = str.match(/^(([1-9]\d*)|(([0-9]|[1-9]+)\.[0-9]{1,2}))$/);
...
function isUrl(str) {
  ...
  let result = str.match(/^http:\/\/[A-Za-z0-9]+\.[A-Za-z0-9]+[\/=\?%\-&_~`@[\]\’:+!]*([^<>\"])*$/);
...
function isPwd(str) {
  ...
  let result = str.match(/^[a-zA-Z]\\w{6,12}$/);
...
function stringCheck(str) {
  ...
  let result = str.match(/^[a-zA-Z0-9\u4e00-\u9fa5-_]+$/);
